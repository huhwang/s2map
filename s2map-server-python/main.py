...
if __name__ == "__main__":
    ...
    is the port. If only one arg appears, it is assumed as port num. For port, an integer
    type is expected. If it's not the case, an exception will be thrown'''
    ARGS = sys.argv
    if len(ARGS) == 2:
        start_server('localhost', int(ARGS[1]))
    elif len(ARGS) == 3:
